...
async def getworkdays(user_id, month, total):
    # user = await db_api.sel_user(user_id)
    # mode = user['mode']
    # days = await get_wdays(month, date.today().year)
    # days_plan = days['work']
    days_fact = 0
    trips = await db_api.sel_time(user_id)
    for note in trips:
...
async def gen_turnover(user_id, month):
    turnover = await db_api.sel_turnover(user_id)
    msg = 'Оборот за месяц\n'
    total = 0
    ...
    days = await getworkdays(user_id, month, total)
    try:
        msg += (f'На данный момент ваша премия составляет '
                f'{days["plan_bonus"]["perc"]}% '
                f'({days["plan_bonus"]["cash"]} р.)\n')
    except Exception:
        pass
    try:
        msg += (f'По кол-ву добавленных раб. дней ваша премия составляет '
                f'{days["fact_bonus"]["perc"]}% '
                f'({days["fact_bonus"]["cash"]} р.)')
    except Exception:
        pass
    return msg
