...
from funcs import gen_turnover
...
def register_handlers_reports_to(dp: Dispatcher):
    dp.register_message_handler(menu, state=Reports.turnover_reported, regexp="Меню")
    dp.register_message_handler(prev_m, state=Reports.turnover_reported, regexp="Предыдущий месяц")
    dp.register_message_handler(another_m, state=Reports.turnover_reported, regexp="Другой месяц")
    dp.register_message_handler(delete, state=Reports.turnover_reported, regexp="Удалить запись")
    dp.register_message_handler(inval, state=Reports.turnover_reported)
