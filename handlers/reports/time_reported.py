...
from funcs import report_time, log
...
def register_handlers_reports_time(dp: Dispatcher):
    dp.register_message_handler(menu, state=Reports.time_reported, regexp="Меню")
    dp.register_message_handler(prev_m, state=Reports.time_reported, regexp="Предыдущий месяц")
    dp.register_message_handler(another_m, state=Reports.time_reported, regexp="Другой месяц")
    dp.register_message_handler(delete, state=Reports.time_reported, regexp="Удалить запись")
    dp.register_message_handler(inval, state=Reports.time_reported)
