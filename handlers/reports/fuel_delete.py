...
def register_handlers_reports_fuel_delete(dp: Dispatcher):
    dp.register_message_handler(delete, state=Reports.fuel_del_confirm, regexp="Удалить записи")
    dp.register_message_handler(menu, state=Reports.fuel_del_confirm, regexp="Меню")
    dp.register_message_handler(inval, state=Reports.fuel_del_confirm)
